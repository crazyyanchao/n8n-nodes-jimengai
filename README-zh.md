...
4. 选择具体操作：
   - 图片生成：文生图2.1、文生图3.0、文生图3.1、图生图3.0、文生图3.0结果查询、文生图3.1结果查询、图生图3.0结果查询
   - 视频生成：文生视频720P/1080P、图生视频720P/1080P、动作模仿、视频生成3.0 Pro等
   - 状态查询：各类图片结果查询、视频结果查询等
...
7. **异步任务处理**：
   - 对于异步接口（文生图3.0/3.1、所有视频生成），会返回任务ID
   - 使用对应的状态查询接口定期检查任务状态
   - 任务完成后获取生成的图像或视频
...
- **结果查询**：文生图3.0结果查询、文生图3.1结果查询、图生图3.0结果查询
...
#### 异步接口（需要状态查询）
- **文生图3.0/3.1**：提交任务后返回任务ID，需要定期查询状态
...
3. **状态查询**：使用对应的结果查询接口定期检查任务状态
   - 图片生成：文生图3.0结果、文生图3.1结果、图生图3.0结果
   - 视频生成：各类视频结果查询接口
4. **获取结果**：任务完成后获取生成的图像或视频
5. **可选配置**：在结果查询时可配置水印和AIGC元数据
