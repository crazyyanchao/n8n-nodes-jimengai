...
const GetImageToVideo720PFirstFrameResultOperate: ResourceOperations = {
	name: 'Image to Video 720P First Frame Result',
	value: 'getImageToVideo720PFirstFrameResult',
	description: 'Get the result of 720P image to video first frame generation task',
	options: [
		{
			displayName: 'Task ID',
			name: 'taskId',
			type: 'string',
			default: '',
			description: 'Task ID of the 720P image to video first frame generation task',
			required: true,
		},
		{
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		this.logger.info('Starting image to video 720P first frame result processing', {
			taskId: taskId,
			outputFormat: outputFormat,
		...
		try {
			this.logger.info('Calling API to get image to video 720P first frame result', { taskId });
			const data = await client.getImageToVideo720PFirstFrameResult(taskId);

			this.logger.info('API response received', {
				code: data.code,
				status: data.data?.status,
				message: data.message,
				hasVideoUrl: !!(data.data?.video_url)
			});
			...
			if (error.message.includes('Server internal error')) {
				throw new Error(`Image to Video 720P First Frame result query failed: Server internal error for task ${taskId}. This may be due to: 1) The task is still being processed, 2) Server temporary issues. Please try again later. If the problem persists, contact support.`);
			}

			if (error.message.includes('Internal Error')) {
				throw new Error(`Image to Video 720P First Frame result query failed: Internal server error for task ${taskId}. The server encountered an unexpected condition. Please try again later.`);
			}

			if (error.message.includes('Task not found')) {
				throw new Error(`Image to Video 720P First Frame result query failed: Task not found - ${taskId}. Please verify the task ID is correct and hasn't expired (tasks expire after 12 hours).`);
			}

			if (error.message.includes('Task has expired')) {
				throw new Error(`Image to Video 720P First Frame result query failed: Task has expired - ${taskId}. Please resubmit the video generation task.`);
			}
...
export default GetImageToVideo720PFirstFrameResultOperate;
