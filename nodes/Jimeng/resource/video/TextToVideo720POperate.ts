...
const TextToVideo720POperate: ResourceOperations = {
	name: 'Text to Video 720P',
	value: 'textToVideo720P',
	description: 'Generate 720P video from text prompt using Jimeng 3.0 model',
	options: [
		{
			displayName: 'Prompt',
			name: 'prompt',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		// Calculate frames based on duration (frames = 24 * n + 1, where n is seconds)
		const frames = 24 * duration + 1;

		const data = await client.textToVideo720P({
			prompt,
...
export default TextToVideo720POperate;
