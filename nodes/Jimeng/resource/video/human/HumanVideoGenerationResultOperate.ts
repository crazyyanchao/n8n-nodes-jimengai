...
function processVideoOutput(
	...
	switch (outputFormat) {
		...
		case 'binary':
			...
			const outputFileName = `digital_human_video.${format}`;
...
const GetHumanVideoGenerationResultOperate: ResourceOperations = {
	name: 'Human Video Generation Result',
	value: 'getDigitalHumanVideoGenerationResult',
	description: 'Get the result of a Digital Human Video Generation task',
	options: [
		{
			displayName: 'Task ID',
			name: 'taskId',
			type: 'string',
			default: '',
			description: 'Task ID returned from Digital Human Video Generation submission',
			required: true,
		},
		{
		...
		},
		{
			...
			name: 'outputFilePath',
			type: 'string',
			default: './output/digital_human_video.mp4',
		...
		},
		{
			...
			name: 'cacheDir',
			type: 'string',
			default: './cache/digital_human_video',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		const cacheDir = enableCache ? (this.getNodeParameter('cacheDir', index) as string) : './cache/digital_human_video';
		...
		this.logger.info('Starting digital human video generation result processing', {
			taskId: taskId,
			outputFormat: outputFormat,
		...
		try {
			this.logger.info('Calling API to get digital human video generation result', { taskId });
			const data = await client.getDigitalHumanVideoGenerationResult(taskId);
			...
			if (error.message.includes('Server internal error')) {
				throw new Error(`Digital Human Video Generation result query failed: Server internal error for task ${taskId}. This may be due to: 1) The task is still being processed, 2) Server temporary issues. Please try again later. If the problem persists, contact support.`);
			}

			if (error.message.includes('Internal Error')) {
				throw new Error(`Digital Human Video Generation result query failed: Internal server error for task ${taskId}. The server encountered an unexpected condition. Please try again later.`);
			}

			if (error.message.includes('Task not found')) {
				throw new Error(`Digital Human Video Generation result query failed: Task not found - ${taskId}. Please verify the task ID is correct and hasn't expired (tasks expire after 12 hours).`);
			}

			if (error.message.includes('Task has expired')) {
				throw new Error(`Digital Human Video Generation result query failed: Task has expired - ${taskId}. Please resubmit the digital human video generation task.`);
			}
...
export default GetHumanVideoGenerationResultOperate;
