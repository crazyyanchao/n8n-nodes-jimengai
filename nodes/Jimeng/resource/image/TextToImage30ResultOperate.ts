...
const TextToImage30ResultOperate: ResourceOperations = {
	name: 'Text to Image 3.0 Result',
	value: 'textToImage30Result',
	description: 'Get the result of a Text to Image 3.0 task using task ID',
	options: [
		{
			displayName: 'Task ID',
			name: 'task_id',
			type: 'string',
			default: '',
			description: 'Task ID returned from Text to Image 3.0 submission',
			required: true,
		},
		{
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		if (!taskId || taskId.trim() === '') {
			throw new Error('Task ID is required. Please provide a valid task ID from the Text to Image 3.0 submission.');
		}
		...
		try {
			const data = await client.getTextToImage30Result(taskId, {
			...
			if (error.message.includes('Task has expired')) {
				throw new Error(`Task has expired. Please resubmit the Text to Image 3.0 task and use the new Task ID.`);
			}
...
export default TextToImage30ResultOperate;
